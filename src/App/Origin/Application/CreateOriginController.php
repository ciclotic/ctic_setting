...
use CTIC\App\Origin\Infrastructure\Controller\OriginController;
use CTIC\App\Origin\Infrastructure\Form\Type\OriginType;
...
class CreateOriginController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new OriginType(Origin::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\App\Origin\Infrastructure\Form\Type\OriginType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateOrigin();
        }
        if(empty($command->resource))
        {
            $command->resource = Origin::class;
        }
        if(empty($command->command))
        {
            $command->command = new OriginCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return OriginController::class;
    }
