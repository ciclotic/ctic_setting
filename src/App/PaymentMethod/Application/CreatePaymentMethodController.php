...
use CTIC\App\PaymentMethod\Infrastructure\Controller\PaymentMethodController;
use CTIC\App\PaymentMethod\Infrastructure\Form\Type\PaymentMethodType;
...
class CreatePaymentMethodController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new PaymentMethodType(PaymentMethod::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\App\PaymentMethod\Infrastructure\Form\Type\PaymentMethodType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreatePaymentMethod();
        }
        if(empty($command->resource))
        {
            $command->resource = PaymentMethod::class;
        }
        if(empty($command->command))
        {
            $command->command = new PaymentMethodCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return PaymentMethodController::class;
    }
