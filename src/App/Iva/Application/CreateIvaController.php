...
use CTIC\App\Iva\Infrastructure\Controller\IvaController;
use CTIC\App\Iva\Infrastructure\Form\Type\IvaType;
...
class CreateIvaController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new IvaType(Iva::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\App\Iva\Infrastructure\Form\Type\IvaType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateIva();
        }
        if(empty($command->resource))
        {
            $command->resource = Iva::class;
        }
        if(empty($command->command))
        {
            $command->command = new IvaCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return IvaController::class;
    }
