...
use CTIC\App\Bank\Infrastructure\Controller\BankController;
use CTIC\App\Bank\Infrastructure\Form\Type\BankType;
...
class CreateBankController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new BankType(Bank::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\App\Bank\Infrastructure\Form\Type\BankType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateBank();
        }
        if(empty($command->resource))
        {
            $command->resource = Bank::class;
        }
        if(empty($command->command))
        {
