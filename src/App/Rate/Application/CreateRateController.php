...
use CTIC\App\Rate\Infrastructure\Controller\RateController;
use CTIC\App\Rate\Infrastructure\Form\Type\RateType;
...
class CreateRateController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new RateType(Rate::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\App\Rate\Infrastructure\Form\Type\RateType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateRate();
        }
        if(empty($command->resource))
        {
            $command->resource = Rate::class;
        }
        if(empty($command->command))
        {
            $command->command = new RateCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return RateController::class;
    }
