...
use CTIC\App\RealizationArea\Infrastructure\Controller\RealizationAreaController;
use CTIC\App\RealizationArea\Infrastructure\Form\Type\RealizationAreaType;
...
class CreateRealizationAreaController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new RealizationAreaType(RealizationArea::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\App\RealizationArea\Infrastructure\Form\Type\RealizationAreaType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateRealizationArea();
        }
        if(empty($command->resource))
        {
            $command->resource = RealizationArea::class;
        }
        if(empty($command->command))
        {
            $command->command = new RealizationAreaCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return RealizationAreaController::class;
    }
